...
func DealRequestFromRepo(w http.ResponseWriter, v interface{}, status int) {
	switch status {
	case repository.DB_ERROR:
	...
	case repository.CONFLICT:
		errText := models.Error{Message: "conflict happened while performing these actions"}
		WriteToResponse(w, http.StatusConflict,
			errText)
		return
	case repository.WRONG_INPUT:
		errText := models.Error{Message: "input is incorrect"}
		WriteToResponse(w, http.StatusBadRequest, errText)
	}
}
